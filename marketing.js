...
form.addEventListener('submit',async e=>{e.preventDefault();if(!form.reportValidity())return;const button=form.querySelector('button[type=submit]'),status=document.querySelector('#contact-status');button.disabled=true;button.textContent='Sending…';status.textContent='';try{const response=await fetch(form.action,{method:'POST',headers:{Accept:'application/json','Content-Type':'application/json'},body:JSON.stringify(Object.fromEntries(new FormData(form)))});if(!response.ok)throw Error('Submission failed');status.textContent='Thanks — your inquiry has been sent. Nina will be in touch personally.';playDelivery(form.elements.name.value.trim().split(' ')[0]);form.reset();document.querySelector('#plan-note').hidden=true}catch{status.textContent='Your inquiry could not be sent. Please try again or email nina@violetinvestigations.com. Your details are still here.'}finally{button.disabled=false;button.innerHTML='Send inquiry <span class="cta-arrow" aria-hidden="true">↗</span>'}})})();
