...
const chapters=[
 ['TELL VIOLET','A conversation.<br>A starting point.','Elena Marquez shares a concern with Violet by voice or chat. The conversation becomes the starting point for triage.','The voice screen shown here follows Tell Violet’s actual interface. This is a simulated call.','Tell Violet · Voice intake'],
...
function hydrateGuidedReport(){
 ...
 report.innerHTML=`${guidedHeader}<div class="guided-body">${guidedNav('Draft')}<div id="draft-state" class="report-state"><div class="guided-panel-title"><div><h4>Draft</h4><p>Build and review the report section by section.</p></div><div class="draft-progress"><strong>9 of 9 drafted</strong><button class="app-button" id="verify-draft" type="button">Verify draft</button></div></div><div class="report-section-list">${['Introduction','Summary of Findings','Investigative Methodology','Documentation','Background','Facts','Findings: Anti-Discrimination Policy','Findings: Anti-Retaliation Policy','Recommendations'].map((label,i)=>`<div><span>✓</span><strong>${label}</strong><small>${i<6?'Drafted':'Reviewed'}</small></div>`).join('')}</div></div><div id="flag-state" class="report-state" hidden>${guidedNav('Flag review')}<div class="flag-summary"><span>ⓘ 1 item to review</span><button class="app-outline" id="show-draft-state" type="button">Back to draft</button></div><div class="flag-review-grid"><article class="draft-passage"><span>VI. FACTS</span><h4>Promotion decision</h4><p>On February 3, Victor Hale announced that Daniel Cho had been selected to lead the Northstar launch. Cho was described in the draft as <mark>Director of Technical Implementation</mark>.</p><p>Elena Marquez had also been considered for the role. The investigator reviewed the candidate matrix, meeting notes, and witness interviews.</p></article><aside class="accuracy-flag"><span class="flag-label">ACCURACY FLAG</span><h4>Daniel Cho’s title needs correction</h4><p>The source identifies Daniel as <strong>Senior Director of Technical Implementation</strong>.</p><div class="source-beside"><span>SOURCE · FINALIST MEETING NOTES</span><blockquote>“Daniel Cho, Senior Director of Technical Implementation, will lead the Northstar launch.”</blockquote></div><div class="flag-actions"><button type="button">Edit this sentence</button><button type="button">Remove the sentence</button><button class="address-flag" type="button">Mark flag addressed</button></div></aside></div></div></div>`;
}
hydrateGuidedReport();
function hydrateHaleCase(){
 const replacements=[
  ...
  [/After I raised a concern, I stopped being invited to the team meetings\./g,'After I questioned the promotion decision, my Northstar summit invitation was canceled.'],[/When did you first notice the invitations had stopped\?/g,'What was said when Daniel was selected for the role?'],
...
function hydrateHaleIntelligence(){
 const shell=(active,content)=>`<div class="product-nav"><b>Violet</b><span>▦ Dashboard</span><span class="nav-active">▱ Cases</span><span>▤ Policies</span><span>☑ Tasks</span><span>▥ Reports</span></div><div class="product-body"><div class="case-heading"><small>‹ Back to Cases</small><h3>Victor Hale</h3><span>INV-2026-0062</span></div><div class="case-tabs"><span>Summary</span><span>Intake</span><span>Investigation</span><span class="${active==='Timeline'?'tab-active':''}">Timeline</span><span class="${active==='Evidence Matrix'?'tab-active':''}">Evidence Matrix</span><span>Documents</span><span>Resolution</span></div>${content}</div>`;
...
$('#verify-draft').addEventListener('click',showReportReview);$('#show-draft-state').addEventListener('click',runReportSequence);
