...
function hydrateGuidedReport(){
 ...
 report.innerHTML=`${guidedHeader}<div class="guided-body">${guidedNav('Draft')}<div id="draft-state" class="report-state"><div class="guided-panel-title"><div><h4>Drafting the report</h4><p>Watch Violet assemble all nine sections, then verify claims against the record.</p></div><div class="draft-progress"><strong>9 of 9 drafted</strong><button class="app-button" id="verify-draft" type="button">Verify now</button></div></div><div class="draft-motion" aria-hidden="true"><i></i><span>Draft complete · beginning source review</span></div><div class="report-section-list">${['Introduction','Summary of Findings','Investigative Methodology','Documentation','Background','Facts','Findings: Anti-Discrimination Policy','Findings: Anti-Retaliation Policy','Recommendations'].map((label,i)=>`<div style="--row:${i}"><span>✓</span><strong>${label}</strong><small>${i<6?'Drafted':'Reviewed'}</small></div>`).join('')}</div></div><div id="flag-state" class="report-state" hidden>${guidedNav('Flag review')}<div class="flag-summary"><span>ⓘ 1 item to review · complete report shown below</span><div><button class="app-outline" id="open-full-report" type="button">Open full report</button><button class="app-outline" id="show-draft-state" type="button">Replay animation</button></div></div><div class="flag-review-grid"><article class="draft-passage full-report-preview">${reportHtml.replace('Senior Director of Technical Implementation','<mark>Director of Technical Implementation</mark>')}</article><aside class="accuracy-flag"><span class="flag-label">ACCURACY FLAG</span><h4>Daniel Cho’s title needs correction</h4><p>The source identifies Daniel as <strong>Senior Director of Technical Implementation</strong>.</p><div class="source-beside"><span>SOURCE · FINALIST MEETING NOTES</span><blockquote>“Daniel Cho, Senior Director of Technical Implementation, will lead the Northstar launch.”</blockquote></div><div class="flag-actions"><button type="button">Edit this sentence</button><button type="button">Remove the sentence</button><button class="address-flag" type="button">Mark flag addressed</button></div></aside></div></div></div>`;
}
hydrateGuidedReport();
...
function runReportSequence(){clearTimeout(reportTimer);$('#draft-state').hidden=false;$('#flag-state').hidden=true;setReportNav('Draft');$('#draft-state').classList.remove('sequence-running');void $('#draft-state').offsetWidth;$('#draft-state').classList.add('sequence-running');reportTimer=setTimeout(showReportReview,reduced.matches?1800:5200)}
function stopSpeech(){}
...
$('#verify-draft').addEventListener('click',showReportReview);$('#show-draft-state').addEventListener('click',runReportSequence);$('#open-full-report').addEventListener('click',openReport);
